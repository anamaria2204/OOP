...
map<string, DTO> OfertaService::raport() noexcept {
	...
	for (const auto& oferta : oferte) {
		DTO denumire(oferta.get_denumire());
		rap[oferta.get_denumire()] = denumire;
	}

	for (const auto& oferta : oferte) {
		int nr = rap[oferta.get_denumire()].get_count() + 1;
		DTO valoare(oferta.get_denumire(), nr);
		rap[oferta.get_denumire()] = valoare;
	}
	return rap;
}