...
void test_cauta_repo() {
	...
	int poz = repo.cauta("vacanta", "Timisoara");
	assert(repo.getAll()[poz].get_denumire() == o1.get_denumire());
	try {
...
void test_repo_file() {
	std::ofstream out("test_oferte.txt", std::ios::trunc);
	out.close();
	RepoFile repF( "test_oferte.txt" );
	repF.store(Oferta{ "vacanta", "Timisoara", "sejur", 1200 });

	RepoFile repF2{ "test_oferte.txt" };
	int poz = repF2.cauta("vacanta", "Timisoara");
	assert(repF2.getAll()[poz].get_pret() == 1200);
	try {
		repF2.cauta("vacanta", "Mamaia");
		assert(false);
	}
	catch (OfertaRepoException&) {
		assert(true);
	}
	repF2.sterge(Oferta{ "vacanta", "Timisoara", "sejur", 1200 });
	assert(repF2.getAll().size() == 0);
	try {
		repF2.sterge(Oferta{ "vacantaa", "Timisoaraa", "sejur", 1200 });
		assert(false);
	}
	catch (OfertaRepoException&) {
		assert(true);
	...
	RepoFile repF3{ "test_oferte.txt" };
	assert(repF3.getAll().size() == 0);

	repF3.store(Oferta{ "vacanta", "Sibiu", "oras", 1200 });
	Oferta of{ "vacanta", "Sibiu", "oras", 1200 };
	repF3.modifica(of, "calatorie", "Mamaia", "mare", 3000);
	assert(repF3.getAll().size() == 1);

	try {
		RepoFile repF4{ "test_ofert/a.txt" };
		assert(false);
	}
	catch (OfertaRepoException&) {
		assert(true);
...
void test_undo() {
	...
	serv.undo();
	assert(serv.getAll().size() == 2);
	serv.modificare(0, "calatorie", "Bucuresti", "oras", 3000);
	assert(serv.getAll()[0].get_denumire() == "calatorie");
	serv.undo();
	assert(serv.getAll()[0].get_denumire() == "vacanta");
}
